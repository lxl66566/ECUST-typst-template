...
内部用模块 #link("https://typst.app/universe/package/i-figured")[i-figure] 实现图片，表格与公式的编号。
...
引用的话就在 figure 后加上标签，在原标签前加上前缀 `fig:, tbl:, eqt:, lst:`（分别对应图片，表格、公式与代码块）。
...
#figure(
  table(
    ...
    image("assets/avatar.jpeg", height: 10%), [$ sqrt(2) / 12 a^3 $ <->], [$a$: 边长],
  ),
  caption: "芝士样表",
) <tbl1>
...
但是一个很长的带有 figure 的代码块无法跨页，此时需要用下面的形式。
...
（比较奇怪的是在全局设置 `#show figure.where(kind: raw): set block(breakable: true)` 是无效的，所以需要这样让用户手动操作，希望有其他 typst 佬解惑）
...
引用支持 LaTeX Bib 的格式。在引用时使用 `@tag`，像这样@impagliazzo2001problems @Burckhardt:2013 以获得右上的引用标注 @刘星2014恶意代码的函数调用图相似性分析 @papadimitriou1998combinatorial。
